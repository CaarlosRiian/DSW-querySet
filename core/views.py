...
def query_examples(request):
    ...
    books_by_title = Book.objects.filter(title__icontains='j')

    # Consulta com lookup para buscar autores por nome
    authors_by_name = Author.objects.filter(name__icontains='Nicolas')
    ...
    books_of_author = Book.objects.filter(author__name='Daniel')

    # Consulta agregada (por exemplo, número de livros de um autor)
    num_books_of_author = Book.objects.filter(author__name='Felipe').count()
